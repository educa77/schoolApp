...
function sendEmail(user, type, data) {
  var modelEmail = fs.readFileSync(
    "./src/mailModels/basicModel.html",
    "utf8",
    function (err, data) {
  ...
  if (type === "passwordReset") {
    ...
    linkTemplate = `<a class="boton" style="background-color: #000;font-weight: bold;color:#fff;box-shadow: 0 0 4px 1px #fafc01" href="${
    ...
    linkTemplate = `<a class="boton" style="background-color: #000;font-weight: bold;color:#fff;box-shadow: 0 0 4px 1px #fafc01" href="${
      process.env.CALLBACK_URL_BASE || "http://localhost:3000"
    }/auth/signup?token=${token}">ÚNETE</a>`;
  }

...
const include = [{ model: Role, as: "role" }, Cohorte, Group];
...
const createUser = async ({
  ...
  if (roles) {
    ...
    for (const specificRol of roles) {
      const dbRole = await Role.findOne({ as: "role", where: { name: specificRol } });
      dbRoles.push(dbRole);
    }
    ...
    if (role !== "instructor" && role !== "pm" && role !== "student" && role !== "staff") {
      throw {
        error: {
          name: "ApiCreateError",
          type: "Users Error",
          error: {
            message: "the fields for the role are instructor, pm, alumno and staff",
            type: "wrong request",
            code: 400,
          },
        },
    ...
    const theRole = await Role.findOne({ as: "role", where: { name: role } });
    await user.setRoles(theRole);
  } else {
    const theRole = await Role.findOne({ as: "role", where: { name: "student" } });
    await user.setRoles(theRole);
  }
...
const getAllUsers = async ({ where, limit, offset, order }) => {
  ...
  if (where) {
    ...
    if (where["Role"]) {
      localInclude[0] = { model: Role, as: "role", where: parseWhere(where.Role) };
      delete where.Role;
    }
    if (where["Cohorte"]) {
...
const countUsers = async ({ where }) => {
  ...
  if (where) {
    where = parseWhere(where);
    if (where["Role"]) {
      localInclude = [...localInclude, { model: Role, as: "role", where: parseWhere(where.Role) }];
      delete where.Role;
    }
    if (where["Cohorte"]) {
      localInclude = [...localInclude, { model: Cohorte, where: parseWhere(where.Cohorte) }];
      delete where.Cohorte;
    }
    if (where["Group"]) {
      localInclude = [...localInclude, { model: Group, where: parseWhere(where.Group) }];
      delete where.Group;
    }
  }
